# Hybrid-based Text Summarization
...
To create the summaries, two libraries need to be installed:

1. `evaluate`: Required for evaluating summaries generated by the model with gold summary.

2. `nltk`: Needed for tokenizing.
              ```python
...
1. Run the file `Hybrid_Text_Summarization_Project-Updated.ipynb` in Jupyter Notebook or Colab.
   ...
   **Note:**
   Before running the file, ensure the paths are properly provided for:

   - Reading the files from the `annual_reports` folder and saving/writing the generated summaries to a folder.
   - Renaming the generated summaries by our model to task-specific filenames.
   - The same should be done for gold summaries already generated to maintain a generic way of task representation.

## File Parts:

The runnable file contains four parts:

1. **Summary Generation:**
   - Reads the files from the `annual_reports` folder and saves the generated summaries in the respective folder.

2. **File Renaming:**
   - Performs the renaming of the file for the system-generated and gold summaries to maintain a general naming convention.

3. **Evaluation:**
   - Evaluates generated summaries with gold summaries through a jar file execution.

4. **Scoring:**
   - Calculates average scores for each ROUGE-Types: ROUGE-1, ROUGE-2, ROUGE-L, ROUGE-SU4.
...
You can download the ROUGE 2.0 report from [arXiv](https://arxiv.org/).
